...
local Device = UserInputService.TouchEnabled and "Mobile" or "Computer"
...
RunService.Heartbeat:Connect(function(deltaTime)
    GlowImage.Rotation = GlowImage.Rotation + 90 * deltaTime -- 90 ثانيه 
end)
...
local SizeButton = Create("TextButton", MainFrame, {
    Name = "SizeButton",
...
local SizeMenu = Create("Frame", MainFrame, {
    ...
    Position = UDim2.new(1, -115, 0, 30),
    BackgroundColor3 = Color3.fromRGB(20,20,20),
    Visible = false,
    ZIndex = 49
})

Create("UICorner", SizeMenu, {
    CornerRadius = UDim.new(0,8)
})


local function ChangeSize(x, y)
    local TweenService = game:GetService("TweenService")
...
CreateSizeOption("صغير", 5, 350, 200)
CreateSizeOption("وسط", 35, 420, 250)
CreateSizeOption("كبير", 65, 550, 320)
CreateSizeOption("افتراضي", 95, 420, 250)
...
local SubTitle = InsertTheme(Create("TextLabel", {
    ...
    Text = '<font family="12187370000">'..WMiniText..'</font>',
    RichText = true,
    TextColor3 = Theme["Color Dark Text"],
    BackgroundTransparency = 1,
    TextXAlignment = "Left",
...
local Title = InsertTheme(Create("TextLabel", TopBar, {
    ...
    AnchorPoint = Vector2.new(0, 0.5),
    AutomaticSize = "XY",
    Text = '<font family="12187370000">'..WTitle..'</font>',
    RichText = true,
    TextXAlignment = "Left",
    TextSize = 12,
    TextColor3 = Theme["Color Text"],
...
task.spawn(function()
    while TimerLabel and TimerLabel.Parent do
        ...
        TimerLabel.Position = UDim2.new(1, SubTitle.AbsoluteSize.X + 19, 0.9)
        task.wait(1)
    end
end)
...
Glow.ImageColor3 = Theme["Color Theme"]
...
local function IconEffect(img)
    ...
    CreateTween({Selected, "BackgroundTransparency", 0, 0.35})
		...
		function Tab:AddSection(Configs, IconConfig)
    local SectionName = type(Configs) == "string" and Configs or Configs[1] or Configs.Name or Configs.Title or "Section"
    local Icon = nil
    
    if IconConfig then
        Icon = IconConfig.Image or IconConfig.Icon or IconConfig[1]
        if Icon and not Icon:find("rbxassetid://") then
            Icon = redzlib:GetIcon(Icon)
        end
    end
    
    local SectionFrame = Create("Frame", Container, {
        Size = UDim2.new(1, 0, 0, Icon and 32 or 20),
        BackgroundTransparency = 1,
        Name = "Option"
    })
    
    if Icon then
        local IconImage = Create("ImageLabel", SectionFrame, {
    ...
    local DefaultIcon = "rbxassetid://10709791437"
    local ClickIcon = "rbxassetid://10709769508"
    local Busy = false

    FButton.Activated:Connect(function()
        if Busy then return end
        Busy = true
        ButtonIcon.Image = ClickIcon
        local startPos = ButtonIcon.Position
        local t = 0
        local c
        c = game:GetService("RunService").RenderStepped:Connect(function(dt)
            t += dt
            local offset = math.sin(t * 40) * 3
            ButtonIcon.Position = startPos + UDim2.new(0, offset, 0, 0)
            if t >= 1 then
                c:Disconnect()
                ButtonIcon.Position = startPos
                ButtonIcon.Image = DefaultIcon
                Busy = false
            end
        end)
...
function Tab:AddInfoPlayer()
    ...
    TextStroke.Color = Theme["Color Stroke"]
    ...
    local InfoPlayer = {}
    function InfoPlayer:Visible(...)
        Funcs:ToggleVisible(Frame, ...)
    end
    function InfoPlayer:Destroy()
        Frame:Destroy()
    end
    return InfoPlayer
end

function Tab:AddButtonTime(Configs)
    local BDescription = Configs[1] or Configs.Desc or Configs.Description or ""
    local Callback = Funcs:GetCallback(Configs, 2)
    local FButton, LabelFunc = ButtonFrame(Container, BDescription, "00:00:00", UDim2.new(1, -20))
    ...
    task.spawn(function()
        while FButton and FButton.Parent do
            local elapsed = tick() - StartTime
            local h = math.floor(elapsed / 3600)
            local m = math.floor((elapsed % 3600) / 60)
            local s = math.floor(elapsed % 60)
            LabelFunc:SetDesc(string.format("%02d:%02d:%02d", h, m, s))
            task.wait(1)
        end
    end)
    ...
    function Button:ResetTime()
        StartTime = tick()
    end
    return Button
end

function Tab:AddButtonFPS(Configs)
    local BDescription = Configs[1] or Configs.Desc or Configs.Description or ""
    local Callback = Funcs:GetCallback(Configs, 2)
    local FButton, LabelFunc = ButtonFrame(Container, BDescription, "FPS: 0", UDim2.new(1, -20))
    ...
    local ButtonIcon = Create("ImageLabel", FButton, {
        ...
        AnchorPoint = Vector2.new(1, 0.5),
        BackgroundTransparency = 1,
        Image = "rbxassetid://95967503310967"
    })
    
    local DefaultIcon = "rbxassetid://95967503310967"
    local ClickIcon = "rbxassetid://95967503310967"
    local Busy = false
    local RunService = game:GetService("RunService")
    local FPS = 0
    ...
    Connection = RunService.Heartbeat:Connect(function()
        if not FButton or not FButton.Parent then
    ...
    return Button
		...
		function Tab:AddImageUsers(Configs)

    local Title = Configs.Title or "Users"
    local Background = Configs.Background or ""

    local Image1 = Configs.Image1 or ""
    local Image2 = Configs.Image2 or ""
    local Image3 = Configs.Image3 or ""

    local Copy1 = Configs.Copy1 or ""
    local Copy2 = Configs.Copy2 or ""
    local Copy3 = Configs.Copy3 or ""


    local Holder = Create("Frame", Container, {
        Size = UDim2.new(1,0,0,220),
        BackgroundColor3 = Color3.fromRGB(0,0,0),
        BorderColor3 = Color3.fromRGB(170,0,255),
        BorderSizePixel = 2
    })
    ...
    local BgImage = Create("ImageLabel", Holder, {
    ...
    Create("UICorner", BgImage, {
        CornerRadius = UDim.new(0,12)
    })

    local function MakeUser(Pos, Img, CopyText)

        local Image = Create("ImageLabel", Holder, {
        ...
        Create("UICorner", Image, {
            CornerRadius = UDim.new(1,0)
        })

        local Button = Create("TextButton", Holder, {
            Size = UDim2.new(0,70,0,28),
            Position = UDim2.new(Pos.X.Scale, Pos.X.Offset-5, 0, 150),
    ...
    MakeUser(UDim2.new(0.2,-30,0,80), Image1, Copy1)
    MakeUser(UDim2.new(0.5,-30,0,80), Image2, Copy2)
    MakeUser(UDim2.new(0.8,-30,0,80), Image3, Copy3)
    ...
    local IconImage = Configs.Icon or ""
    local IconTransparency = Configs.IconTransparency or 0

    local FButton, LabelFunc = ButtonFrame(Container, BName, BDescription, UDim2.new(1, -20))


    local LeftIcon = Create("ImageLabel", FButton, {
    ...
    function Button:SetIcon(ImageId)
        LeftIcon.Image = ImageId
    end

    function Button:SetTransparency(Value)
        LeftIcon.ImageTransparency = Value
    end
    ...
    local Glow = Instance.new("ImageLabel")
    Glow.Parent = FrameHolder
    Glow.Size = UDim2.new(1.15, 0, 1.25, 0)
    Glow.Position = UDim2.new(-0.075, 0, -0.125, 0)
    Glow.BackgroundTransparency = 1
    Glow.Image = "rbxassetid://5028857084"
    Glow.ImageColor3 = Theme["Color Theme"]
    Glow.ImageTransparency = 1
    Glow.ZIndex = FrameHolder.ZIndex - 1
    ...
    Pulse.Parent = JoinButton
    Pulse.Size = UDim2.new(1.8, 0, 2.2, 0)
    Pulse.Position = UDim2.new(-0.4, 0, -0.6, 0)
    Pulse.BackgroundTransparency = 1
    Pulse.Image = "rbxassetid://5028857084"
    Pulse.ImageColor3 = Theme["Color Stroke"]
    Pulse.ImageTransparency = 0.6
    Pulse.ZIndex = JoinButton.ZIndex - 1
...
local function AutoApplyRotatingBorders(Speed)
   Speed = Speed or 80
